...
class KVCacheConfigurator:
    ...
    def _build_hybrid_req_pool(
        ...
        *,
        ...
        # DSPARK/DFLASH used to scatter via update_mamba_state_after_mtp_verify.
        # Overlay worker now commits ReplaySSM through spec_utils GDN fold
        # (same as EAGLE), so hybrid GDN is valid. Still refuse other mamba-ish.
        ...
        if (
            ...
            raise ValueError(
                ...
                "(kimi_linear) or hybrid GDN model; got a non-KDA/non-GDN model."
            )
        req_to_token_pool = HybridReqToTokenPool(
            size=max_num_reqs,
            mamba_size=get_schedule().max_mamba_cache_size,
