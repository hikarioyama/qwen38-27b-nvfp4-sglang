...
try:
    ...
    from sglang.srt.layers.logprob_processor import (
        compute_spec_v2_logprobs as compute_spec_logprobs,
    )
...
try:
    from sglang.srt.speculative.spec_utils import make_linear_accept_index
except ImportError:

    def make_linear_accept_index(accept_lens, draft_token_num):
...
class DFlashWorkerV2(BaseSpecWorker):
    ...
    def init_attention_backends(self):
        ...
        self._need_mamba_verify_commit = mambaish_config(
            ...
            or bool(getattr(self.server_args, "enable_linear_replayssm_spec", False))
        )
    ...
    def _propose_selector_block(
        ...
        *,
        ...
        elif __import__("os").environ.get("SGLANG_DFLASH_DUMP_RECALL") and self._draft_sampler is not None:
            self._draft_sampler.candidate_out[:bs].copy_(candidate_ids)
    ...
    def _update_target_mamba_state_after_verify(
        ...
        *,
        ...
        # ReplaySSM spec-verify: fold the accepted prefix through spec_utils
        # (same path as EAGLE). The flag sets intermediate_ssm=None, so the
        # MTP scatter below would crash or leave GDN state stale.
        if getattr(self.server_args, "enable_linear_replayssm_spec", False):
            seq_lens_now = batch.seq_lens
            if seq_lens_pre_verify is not None:
                # spec_utils treats batch.seq_lens as the pre-verify prefix.
                batch.seq_lens = seq_lens_pre_verify
            try:
                accept_index = make_linear_accept_index(
                    commit_lens, int(self.block_size)
                )
                commit_mamba_states_after_verify(
                    self.target_worker,
                    batch,
    ...
    def forward_batch_generation(
        ...
        if __import__("os").environ.get("SGLANG_DFLASH_DUMP_RECALL") and target_predict is not None and self._draft_sampler is not None:
            cands = self._draft_sampler.candidate_out[:bs]          # [bs, 7, 16]
            tgt = target_predict[:, :-1]
            r1 = candidates[:, 1:] == tgt
            r16 = (cands == tgt.unsqueeze(-1)).any(-1)
            fr = accept_len.to(torch.long).clamp(max=tgt.shape[1] - 1)
            fr_ids = tgt[torch.arange(bs, device=tgt.device), fr]
            logger.info(
                "DFLASH recall r1=%.4f r16=%.4f n=%d first_reject_ids=%s",
                float(r1.float().mean()), float(r16.float().mean()),
                int(r1.numel()), fr_ids.tolist()[:8],
            )
