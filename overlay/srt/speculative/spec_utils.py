...
def make_linear_accept_index(
    accept_lens: torch.Tensor,
    draft_token_num: int,
) -> torch.Tensor:
    """Sequential accept_index for a linear (topk=1) draft chain.

    EAGLE tree verify stores node ids in ``accept_index``; DFLASH/DSPARK
    linear chains are just ``range(bs * K)`` viewed as ``[bs, K]``.
    """
